...
function ReportModal({
  ...
  const handleDownload = async () => {
    const node = document.getElementById('esg-report-print-area');
    if (!node) return;
    ...
    try {
      ...
      pdf.save(`${period.replace(/\s+/g, '-')}-ESG-Report.pdf`);
    } finally {
        ...
        <div className="sticky top-0 bg-white border-b border-slate-100 rounded-t-3xl px-4 sm:px-6 pt-5 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 z-10">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-bold text-white bg-emerald-700 hover:bg-emerald-800 disabled:opacity-60 disabled:cursor-wait px-3 py-2 rounded-xl transition-all"
            >

  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-3xl border border-slate-200 shadow-sm">
        ...
        <Link
          href="/"
          className="inline-flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs px-5 py-3 rounded-2xl transition-all shadow-md hover:scale-[1.02] shrink-0 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2 w-full sm:w-auto"
        >
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3">
        ...
        {/* Sort */}
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2 min-w-0">
          <MapPin className="w-4 h-4 text-slate-400 shrink-0" />
          <select
            id="sort-select"
            value={sortMode}
            onChange={(e) => setSortMode(e.target.value as SortMode)}
            className="text-xs font-semibold text-slate-700 bg-transparent focus:outline-none pr-2 min-w-0 w-full sm:w-auto"
          >
            {SORT_OPTIONS.map((o) => (
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border text-xs font-semibold transition-colors ${
      ...
      {mounted && soldOutListings.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-5 opacity-60">
